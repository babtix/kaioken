...
func TestComposerMultilineGrowsAndShrinksViewport(t *testing.T) {
	...
	if singleLineVP != 30-1-1 {
		t.Fatalf("single-line viewport height = %d, want %d", singleLineVP, 28)
	}
...
func TestComposerEnterSubmits(t *testing.T) {
	...
	if m.vp.Height != 30-1-1 {
		t.Errorf("viewport should return to full height after submit, got %d", m.vp.Height)
	}
}
