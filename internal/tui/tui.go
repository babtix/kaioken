...
// syncLayout re-sizes the viewport around the composer, which grows and
// shrinks as the user types a multi-line message.
func (m *Model) syncLayout() {
	...
	h := m.inputHeight()
	m.input.SetHeight(h)
	// composer rows + one hint line, plus the palette when it is open
	m.vp.Height = max(m.height-h-1-m.paletteHeight(), 1)
	m.refreshViewport()
}
...
// printStatusPanel reprints the Version/Repo/Model/Provider/API-Key block
// that opens the session, so a /model, /provider or /key change is reflected
// somewhere other than a one-line confirmation that scrolls away. The splash
// banner at the very top of scrollback is drawn once and left as history.
func (m *Model) printStatusPanel() {
	m.appendLine("")
	for _, l := range statusPanel(m.cfg, m.repo, m.client != nil) {
		m.appendLine(l)
	}
}
