...
// /model, /provider and /key must all reprint the status panel so a change is
// visible somewhere other than a one-line confirmation that scrolls away —
// the splash banner at the top of scrollback is drawn once and never updates.
func TestProviderAndModelChangesReprintStatusPanel(t *testing.T) {
	m := newTestModel(t)
	before := len(m.lines)
	...
	if !strings.Contains(joined, "Model:") || !strings.Contains(joined, "some/other-model") {
		t.Errorf("/model should reprint an updated status panel, got:\n%s", joined)
	}
	...
	m = updated.(Model)
	joined = strings.Join(m.lines[before:], "\n")
	if !strings.Contains(joined, "Provider:") || !strings.Contains(joined, "groq") {
		t.Errorf("/provider should reprint an updated status panel, got:\n%s", joined)
	}
}
