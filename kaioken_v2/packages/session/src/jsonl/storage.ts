...
import { invalidFile, JsonlDecodeError } from "./errors.js";
import type { JsonlSessionMetadata, JsonlSessionRepoFileSystem, JsonlV4Header } from "./types.js";
...
export class JsonlSessionStorage implements SessionStorage<JsonlSessionMetadata> {
	...
	static async create(
		...
		const fileInfo = await fs.fileInfo(path);
	...
	static async load(fs: JsonlSessionRepoFileSystem, path: string): Promise<JsonlSessionStorage> {
		...
		for (let index = 1; index < physicalLines.length; index++) {
			...
			if (!mutationResult.ok) {
				...
				if (isTornTail) {
					// Drop the partial trailing line from a mid-write crash and repair file cleanly
