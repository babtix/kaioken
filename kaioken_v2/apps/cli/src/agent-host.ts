...
import { createNodeFsPort } from "./ports/fs-port.js";
import { createNodeShellPort } from "./ports/shell-port.js";

/**
 * Create a concrete ExtensionPort bound to the host filesystem.
 */
export function createNodeExtensionPort(workspaceRoot: string): ExtensionPort {
	const loader = new ExtensionLoader({ workspaceRoot });
	const runner = new ExtensionRunner(loader);
	return createExtensionPort(runner);
}

// Re-export port creators for convenience
export { createNodeFsPort, createNodeShellPort };
...
/**
 * Map a KnowledgeTool through paramsToJsonSchema into Pi's AgentTool envelope.
 *
 * Uses standard JSON Schema generated by @kaioken/agent paramsToJsonSchema,
 * avoiding direct TypeBox construction dependencies.
 */
export function knowledgeToolToRuntime(
	tool: KnowledgeTool,
	...
	return {
		...
		parameters: paramsToJsonSchema(tool.params) as import("@earendil-works/pi-ai").TSchema,
		async execute(
			_id: string,
			params: unknown,
			signal?: AbortSignal,
...
/**
 * Map a CodingTool with JSON Schema inputSchema into Pi's AgentTool envelope.
 */
export function codingToolToRuntime(
	tool: CodingTool,
): import("@earendil-works/pi-agent-core").AgentTool {
	return {
		name: tool.name,
		label: tool.label,
		description: tool.description,
		parameters: tool.inputSchema as import("@earendil-works/pi-ai").TSchema,
		async execute(
			_id: string,
			params: unknown,
			signal?: AbortSignal,
...
/**
 * Translate knowledge tools and optional coding tools into the runtime's shape.
 *
 * Keeps backwards compatibility with `toRuntimeTools(ai, tools, ctx)` while
 * supporting additional coding tools.
 */
export function toRuntimeTools(
	_ai: PiAi,
	tools: readonly KnowledgeTool[],
	ctx: KnowledgeContext,
	codingTools: readonly CodingTool[] = [],
): import("@earendil-works/pi-agent-core").AgentTool[] {
	return [
		...tools.map((tool) => knowledgeToolToRuntime(tool, ctx)),
		...codingTools.map((tool) => codingToolToRuntime(tool)),
	];
}

/**
 * Bridge all tools (knowledge + coding + extension tools) bound to ports into Pi's AgentTool envelope.
 * Wires ExtensionPort to discover tools contributed by trusted extensions at startup.
 */
export async function allRuntimeTools(
	ctx: KnowledgeContext,
	ports: CodingToolPorts & { ext?: ExtensionPort },
): Promise<import("@earendil-works/pi-agent-core").AgentTool[]> {
	const { knowledge, coding } = createAllTools(ctx.root, ports);
	const extPort = ports.ext ?? createNodeExtensionPort(ctx.root);
	const extTools = await extPort.discoverTools();
	return [
		...knowledge.map((tool) => knowledgeToolToRuntime(tool, ctx)),
		...coding.map((tool) => codingToolToRuntime(tool)),
		...extTools.map((tool) => codingToolToRuntime(tool)),
	];
}

/**
 * Synchronous variant of allRuntimeTools when extension tools are already loaded or omitted.
 */
export function allRuntimeToolsSync(
	ctx: KnowledgeContext,
	ports: CodingToolPorts,
	extensionTools: readonly CodingTool[] = [],
): import("@earendil-works/pi-agent-core").AgentTool[] {
	const { knowledge, coding } = createAllTools(ctx.root, ports);
	return [
		...knowledge.map((tool) => knowledgeToolToRuntime(tool, ctx)),
		...coding.map((tool) => codingToolToRuntime(tool)),
		...extensionTools.map((tool) => codingToolToRuntime(tool)),
	];
}
...
export function nodeCommandRunner(): CommandRunner {
	return {
		run(command, { cwd, timeoutMs, signal }): Promise<RunOutcome> {
			const started = Date.now();
			return new Promise<RunOutcome>((settle) => {
				const child = exec(
					command,
					{ cwd, timeout: timeoutMs, signal, maxBuffer: 32 * 1024 * 1024, windowsHide: true },
					(error, stdout, stderr) => {
