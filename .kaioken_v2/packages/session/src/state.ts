...
export class SessionState {
	...
	findEntriesOnBranch(query: EntryQuery & BranchBounds & { start: string }): Entry[] {
		...
		// Returns in chronological (root to leaf) order
	...
	createForkMutations(options: ForkOptions): SessionMutation[] {
		let targetId = options.at;
		if (targetId === undefined) {
			targetId = this.requireLane(options.lane ?? "main");
		}
		if (targetId !== null) {
			this.validateTarget(targetId);
		}

		const copiedEntries = targetId === null ? [] : this.findEntriesOnBranch({ start: targetId });
		const forkLanes: LanePointer[] = [{ lane: "main", leafId: targetId }];
	...
	private *walkToRoot(
		...
		while (current) {
			...
			yield current;
			if (current.id === bounds?.stopAt || current.parentId === null) break;
	...
	private matchesEntryQuery(entry: Entry, query: EntryQuery): boolean {
		return (
			...
			(query.afterSeq === undefined || entry.seq > query.afterSeq)
		);
	}
