...
type Client struct {
	...
	promptToks   int
	completeToks int
	reasonToks   int // reasoning tokens, when the provider reports them separately
	...
	// lastCtxToks is how many tokens the most recent call actually occupied:
	// everything sent plus everything generated. Unlike the counters above it
	// is not cumulative — it is a measurement of one request, which is what
	// compaction needs. See LastContextTokens.
	lastCtxToks  int
	lastCtxKnown bool
...
type usage struct {
	...
	// CompletionTokensDetails carries the OpenAI/OpenRouter-style reasoning
	// report: thinking tokens generated by a reasoning model. Present only
	// when the endpoint understands them; absent everywhere else.
...
func (c *Client) recordUsage(u *usage) {
	...
	if u.CompletionTokensDetails != nil {
		c.reasonToks += u.CompletionTokensDetails.ReasoningTokens
	}
	c.billedToks += billed
	c.cacheReadIn += cacheRead
	c.cacheWriteIn += u.cacheWrite
	// What this one call occupied. billed already excludes any cache reads the
	// provider folded into prompt_tokens, so adding cacheRead back is correct
	// for both reporting styles: OpenAI counts cached tokens inside
	// prompt_tokens, Anthropic reports them alongside it.
	if ctx := billed + cacheRead + u.cacheWrite + u.CompletionTokens; ctx > 0 {
		c.lastCtxToks, c.lastCtxKnown = ctx, true
	}
	if u.Cost != nil {
		c.costUSD += *u.Cost
		c.costKnown = true
...
// LastContextTokens reports how many tokens the most recent completed call
// occupied — prompt (including cache traffic) plus the reply. known is false
// until some response carries usage, which is the case for a brand-new client
// and for the handful of providers that report nothing.
//
// This is the measurement compaction should anchor on. Counting the characters
// in a conversation and dividing is an estimate; this is what the provider
// actually charged for, including the tokenizer's opinion of the tool schemas,
// which no character count can see.
func (c *Client) LastContextTokens() (int, bool) {
	c.usageMu.Lock()
	defer c.usageMu.Unlock()
	return c.lastCtxToks, c.lastCtxKnown
}
...
// ReasoningTokens returns the cumulative reasoning (thinking) tokens the
// provider reported separately from the visible reply. Endpoints that do not
// report them leave this at zero.
func (c *Client) ReasoningTokens() int {
	c.usageMu.Lock()
	defer c.usageMu.Unlock()
	return c.reasonToks
}

// WithModel returns a fresh client for the same endpoint and credentials,
// targeting a different model. Usage counters start at zero, which is what
// lets a per-role model cascade meter each role's spend independently.
